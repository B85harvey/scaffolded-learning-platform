...
function PromptInput({ id, value, readOnly, onChange }: PromptInputProps) {
  ...
  // Adjust height to content on mount and whenever the value changes externally
  useLayoutEffect(() => {
    const ta = ref.current
    if (!ta) return
    ta.style.height = 'auto'
    // scrollHeight is 0 in jsdom — fall back to minHeight
...
export function FramedMode({ slide, warnings, isCommitted }: FramedModeProps) {
        ...
        return (
          <div key={prompt.id}>
            ...
            <div className="mb-2 flex min-w-0 items-start justify-between gap-3">
              <label
                htmlFor={`framed-input-${slide.id}-${prompt.id}`}
            ...
            {/* Input */}
            <PromptInput
              id={`framed-input-${slide.id}-${prompt.id}`}
              value={value}
